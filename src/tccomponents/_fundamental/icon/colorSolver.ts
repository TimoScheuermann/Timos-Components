...
export class Solver {
  ...
  solve(): { values: number[] | null; loss: number; filter: string } {
  ...
  solveNarrow(wide: Record<string, number | number[]>) {
    const A: number = wide.loss as number;
    const c = 2;
    const A1 = A + 1;
  ...
  css(filters: number[] | null) {
    function fmt(idx: number, multiplier = 1) {
      return Math.round(((filters && filters[idx]) || 0) * multiplier);
    }
